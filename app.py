...
def decision(pd_val, cost):
    thr = {2:0.28,3:0.241,5:0.18,10:0.12}.get(cost,0.241)
    if pd_val>=0.25: return "REJECT", thr
    if pd_val>=thr:  return "REVIEW", thr
    return "APPROVE", thr
...
with st.sidebar:
    ...
    home = st.selectbox("Housing situation", ["RENT","MORTGAGE","OWN","OTHER"], index=1,
        help="RENT = renting | MORTGAGE = paying off home loan | OWN = fully owns home")
    dti = st.slider("Monthly debt load (% of income)", 0.0, 60.0, 18.0, 0.5,
        help="What % of monthly income goes to debt repayments. Above 35% is risky.")
    revol_util = st.slider("Credit card usage (%)", 0.0, 100.0, 35.0, 1.0,
        help="How much of their credit card limit they currently use. Above 60% is a red flag.")
    delinq = st.number_input("Late payments in last 2 years", 0, 20, 0)
    inq = st.number_input("New credit applications in last 6 months", 0, 20, 1,
    ...
    term = st.radio("Repayment period", [36, 60], horizontal=True,
        format_func=lambda x: f"{x} months ({x//12} years)")
    installment = st.number_input("Monthly payment ($)", 0, 5000, 398)
    purpose = st.selectbox("What is the loan for?",
        ["Debt consolidation","Credit card payoff","Home improvement",
         "Major purchase","Medical","Small business","Car","Vacation","Other"], index=2)

    st.markdown("---")
    st.markdown("### ⚙️ How strict should the bank be?")
    cost = st.selectbox("Approval strictness", [2,3,5,10], index=1,
        format_func=lambda x: {2:"Lenient — approve more",3:"Standard (recommended)",
                                5:"Conservative",10:"Very strict — almost never approve"}[x])
...
dec, thr = decision(pd_val, cost)
limit = min(loan_amnt, annual_inc*(0.15 if dti>=35 else 0.25))


# ── App ───────────────────────────────────────────────────────────────────────
st.markdown("## 🏦 Credit Risk Simulator")
st.markdown("Fill in the loan application on the left — the model instantly predicts the **chance of default** and gives a **bank decision**.")
st.markdown("---")

# Decision banner
dc = {"APPROVE":"approve","REVIEW":"review","REJECT":"reject"}[dec]
em = {"APPROVE":"✅","REVIEW":"⚠️","REJECT":"❌"}[dec]
sub = {"APPROVE":f"Grade {g} — interest rate {apr} — max loan ${limit:,.0f}",
       "REVIEW": f"Grade {g} — borderline case, needs human review",
...
for label, pp in scenarios:
    ...
    delta = new_pd - pd_val
    new_dec, _ = decision(new_pd, cost)
...
st.dataframe(sens_df.style.applymap(color_change,subset=["Change"]).applymap(color_dec,subset=["New decision"]).set_properties(**{"font-size":"13px"}),
...
""")
st.markdown("---")
st.caption("Model trained on LendingClub dataset — 270,887 loans. HistGradientBoosting + Platt calibration. Test AUC = 0.705.")
