...
def grade(pd_val):
    # Grade bands calibrated to real LendingClub distributions
    # Source: LendingClub grade definitions & Basel II PD mapping
    if pd_val < 0.05:  return "A", "7%"    # Very low risk
    if pd_val < 0.10:  return "B", "11%"   # Low risk
    if pd_val < 0.15:  return "C", "14%"   # Moderate risk
    if pd_val < 0.20:  return "D", "18%"   # Medium-high risk
    if pd_val < 0.25:  return "E", "24%"   # High risk
    return "F", "30%"                       # Very high risk — auto-reject zone
